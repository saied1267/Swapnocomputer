  ...
  return (
    <div id="result-checker-section" className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Left Side: Enter Credentials Form (Hidden when result is shown) */}
        {!showResultCard && (
              ...
              {!needPinVerfication ? (
                ...
                <form onSubmit={handleVerifyPin} className="space-y-4">
                  <div className="p-3 bg-amber-50 text-amber-800 rounded-lg text-xs space-y-1 text-left border border-amber-100">
                    ...
                    <p className="text-slate-600">রেজিস্ট্রেশনকৃত মোবাইল নম্বরে প্রেরিত অথবা রশিদ পত্রে উল্লিখিত গোপন পিন লিখুন।</p>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative shrink-0">
                        <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-slate-900 border border-white text-white flex items-center justify-center font-mono text-[8px] font-bold z-10 shadow-sm">
                          {idx + 1}
                        </span>
                        <div className="w-9 h-9 rounded-full border border-slate-200 bg-white overflow-hidden flex items-center justify-center shadow-xs group-hover:border-indigo-200 transition-colors">
                          {students.find(s => s.roll === performer.roll)?.pictureUrl ? (
                            <img 
                              src={students.find(s => s.roll === performer.roll)?.pictureUrl} 
                              alt={performer.name} 
        ...
        <div className={showResultCard ? "lg:col-span-3 w-full" : "lg:col-span-2 h-full"}>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center no-print">
                <button
                  type="button"
                  onClick={handleReset}
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-bold transition-all shadow-sm cursor-pointer"
                >
