...
export default function AdminPanel({
  ...
  // Tab within Admin Panel: 'database', 'leads', 'add-result', 'notices'
  const [adminTab, setAdminTab] = useState<"database" | "leads" | "add-result" | "notices">("database");
  ...
  const handleAddStudentSubmit = (e: React.FormEvent) => {
    ...
    if (!formRoll || !formName || !formMobile || !formPin) {
      alert("রোল, নাম, মোবাইল এবং ৪-সংখ্যার সিকিউরিটি পিন অবশ্যই দিতে হবে!");
      return;
    }
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl max-w-lg w-full overflow-hidden">
            ...
            <form onSubmit={handleAddStudentSubmit} className="p-6 space-y-4 text-left max-h-[450px] overflow-y-auto">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500">গোপন ৪-ডিজিট পিন (PIN)</label>
                  <input
                    type="text"
                    required
                    maxLength={4}
                    placeholder="যেমন: 4545"
                    value={formPin}
