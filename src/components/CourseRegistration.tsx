...
export default function CourseRegistration({ onAddVisitorMessage }: CourseRegistrationProps) {
  ...
  return (
    <div id="courses-section" className="space-y-12">
      ...
      {/* Online Admission Form on top as requested */}
      <div className="max-w-2xl mx-auto w-full">
        <div className="bg-white rounded-3xl border border-indigo-100 p-6 md:p-8 shadow-md shadow-indigo-50/30 text-left space-y-6 relative">
          <div className="space-y-1.5 border-b border-slate-100 pb-4">
            <h4 className="font-extrabold text-slate-900 text-xl flex items-center gap-1.5">
          ...
          <form onSubmit={handleApply} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1 text-left">
                <label className="text-xs font-semibold text-slate-700">পিতার নাম (Father's Name)</label>
                <input
                  type="text"
                  placeholder="পিতার নাম লিখুন"
                  value={fatherName}
                  onChange={(e) => setFatherName(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-xs text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-sans"
                />
              </div>

              <div className="space-y-1 text-left">
                <label className="text-xs font-semibold text-slate-700">মাতার নাম (Mother's Name)</label>
                <input
                  type="text"
                  placeholder="মাতার নাম লিখুন"
                  value={motherName}
                  onChange={(e) => setMotherName(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-xs text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                />
              </div>

              <div className="space-y-1 text-left">
                <label className="text-xs font-semibold text-slate-700">জন্ম তারিখ (Date of Birth)</label>
                <input
                  type="date"
                  value={dob}
                  onChange={(e) => setDob(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-xs text-slate-800 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                />
              </div>
              ...
              <div className="space-y-1 text-left">
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer transition-all"
                >
                  {COURSES.map(c => (
            ...
            >
              <div className="space-y-4">
                ...
                <div className="space-y-1.5 pt-2">
                  ...
                  <ul className="space-y-1 text-slate-650 text-xs">
